...
Paused snapshots and deletion debt use separate stop confirmation. See
[sandbox stop confirmation and cleanup](sandbox-cleanup.md) for retry behavior,
legacy records, and rollback restrictions.
...
| Artifact | Location | Contents | Purpose | Lifecycle |
| --- | --- | --- | --- | --- |
| Paused sandbox record DB | `<persisted_sandbox_store_path>/records.db` | RocksDB keyed by sandbox ID; values are compact JSON records containing version, lifecycle, sandbox metadata, artifact root, and backend state | Restores paused sandboxes after server restart | Snapshot record written before stop; `runtime_stopped` confirms stop separately. Marked `resuming` before launch or `deleting` before cleanup. Interrupted resumes and deletion debt survive restart. DELETE removes artifacts before the record. |
